...
- 在桌面任务栏中，单击鼠标右键选择 **插件 > 系统监视器**，系统插件区会显示图标 ![icon](../common/system_monitor.png)，将鼠标指针置于该图标上，显示处理器、内存和网络数据；单击该图标，在弹出的缩略窗口中显示处理器、网络、内存、磁盘和进程信息，双击窗口中的某个区域（如网络区域），弹出系统监视器界面并显示对应的详细信息。
