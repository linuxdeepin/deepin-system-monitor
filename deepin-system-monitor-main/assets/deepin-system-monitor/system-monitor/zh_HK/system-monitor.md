...
1. 在系統監視器界面上，單擊左側 **詳細訊息**，右側默認展開處理器的詳細訊息。

![1|monitor](fig/viewdetail.png)

2. 單擊按鈕 ![down](../common/nextdown.svg) 展開下拉菜單選項，可以選擇查看內存、網絡或磁盤的詳細訊息。

![1|monitor](fig/detail.png)

3. 單擊右側 **隱藏詳情** 收起詳情頁。

>![notes](../common/tips.svg)竅門：您也可以在桌面上查看電腦的硬件狀態，具體操作步驟如下：
>1. 右鍵單擊桌面任務欄，在 **插件** 子菜單中勾選 **系統監視器**，系統插件區會顯示圖標 ![icon](../common/system_monitor.png)。
>2. 單擊插件區圖標![icon](../common/system_monitor.png)，桌面上會展示電腦的處理器、網絡、內存、磁盤和進程狀態。
>3. 雙擊各個監視區域，打開應用並顯示對應項的詳細訊息。
