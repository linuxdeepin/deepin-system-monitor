...
public Q_SLOTS:
    ...
    // 验证函数：对比DKapture与/proc数据的差异
#ifdef ENABLE_DKAPTURE
    void compareDKaptureWithProc(int pid, const ProcPidStatm* dkStatm, const ProcPidStat* dkStat);
#endif
...
#ifdef ENABLE_DKAPTURE
    // 增量计算辅助方法
    qulonglong calculateDelta(int pid, const QString &field, qulonglong currentValue);
    void resetProcessDeltaData(int pid);
#endif
